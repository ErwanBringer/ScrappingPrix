...
class ScreenDetector():
    ...
    def find_pixel_from_center(self, r, g, b):
        ...
        x = int(len(self.screen_arr[0]) / 2) if len(self.screen_arr[0]) % 2 == 1 else int(
            len(self.screen_arr[0]) / 2) - 1
        offset = 1
    ...
    def is_sub_image_present_within_coordonates(self, sub_image, x, y):
        for i in range(x, x + len(sub_image)):
            for j in range(y, y + len(sub_image[0])):
                if not self.is_pixel_equal(j, i, sub_image[i - x, j - y, 0], sub_image[i - x, j - y, 1],
                                           sub_image[i - x, j - y, 2]):
                    return False
        return True
